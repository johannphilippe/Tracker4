...
namespace lexertk
{
   ...
   {
       comment_style(std::string single_line_begin_,
                     std::string multi_line_begin_,
                     std::string multi_line_end_) :
           single_line_begin(single_line_begin_),
           multi_line_begin(multi_line_begin_),
           multi_line_end(multi_line_end_)
       {
       }

       // comment styles for common languages
       static const comment_style C()
       { 	return comment_style("//", "/*", "*/"); 		}
       static const comment_style CPP()
       { 	return comment_style::C(); 						}
       static const comment_style RUBY()
       {	return comment_style("#", "=begin", "=end"); 	}
       static const comment_style LUA()
       { 	return comment_style("--", "--[[", "]]--"); 	}

       const std::string single_line_begin;
       const std::string multi_line_begin;
       const std::string multi_line_end;
   };

   struct macro_style
   {
      macro_style(std::string m_begin, std::string m_end) :
          macro_begin(m_begin), macro_end(m_end)
      {}

      static const macro_style C() { return macro_style("#", "\n"); }
      static const macro_style CPP() { return macro_style::C(); }

       const std::string macro_begin;
       const std::string macro_end;
   };

   struct syntax_descriptor
   {
       syntax_descriptor(comment_style c_style, macro_style m_style,
                         std::vector<const char *> &keywords) :
           comments(c_style),
           macro(m_style),
           keyword_set(keywords.begin(), keywords.end())
       {
       }

       syntax_descriptor(comment_style &c_style, macro_style &m_style,
                         const char *keywords[], size_t size) :
           comments(c_style),
           macro(m_style)
       {
       ...
       inline bool has_keyword(std::string &s)
       {
           if(keyword_set.find(s) != keyword_set.end()) {
               return true;
           }
       ...
       macro_style macro;
       std::unordered_set<std::string_view> keyword_set;
   };


    inline bool is_keyword(std::string &str, syntax_descriptor &desc)
    {
        if(desc.has_keyword(str))
            return true;
        return false;
    }
   ...
   namespace details
   {
      ...
      inline bool is_comment(syntax_descriptor &desc, const char *s_itr)
      {
          std::string_view single_str(s_itr, desc.comments.single_line_begin.size());
          std::string_view multi_str(s_itr, desc.comments.multi_line_begin.size());
          return( (single_str == desc.comments.single_line_begin) || (multi_str == desc.comments.multi_line_begin) );
      }

      inline bool is_macro(syntax_descriptor &desc, const char *s_itr)
      {
          std::string_view single_str(s_itr, desc.macro.macro_begin.size());
          return( (single_str == desc.macro.macro_begin) );
      }
   ...
   struct token
   {
      ...
      enum token_type
      {
         ...
         e_sub         = '-', e_div         = '/', e_mul         = '*',
         e_mod         = '%', e_pow         = '^', e_colon       = ':',
         e_whitespace  =  16, e_comment     =  17, e_keyword	 =  18,
         e_unknown 	   =  19, e_macro		=  20,
      };
      ...
      template<typename Iterator>
      inline token& set_macro(const Iterator begin, const Iterator end, const Iterator base_begin = Iterator(0))
      {
          type = e_macro;
          value.assign(begin, end);
          if(base_begin)
      ...
      template <typename Iterator>
      inline token& set_symbol( syntax_descriptor &desc,
                               const Iterator begin, const Iterator end,
                               const Iterator base_begin = Iterator(0),
                               bool can_be_keyword = false)
      {
         type = e_symbol;
         value.assign(begin,end);
         if(can_be_keyword && is_keyword(value,  desc) ) type = e_keyword;
         if (base_begin)
            position = std::distance(base_begin,begin);
         return *this;
      }
      ...
      static inline std::string to_str(token_type t)
      {
         switch (t)
         {
            case e_macro	   : return "MACRO";
            case e_unknown	   : return "UNKNOWN";
            case e_keyword	   : return "KEYWORD";
            case e_whitespace  : return "WHITESPACES_OR_ESCAPES";
   ...
   public:
      ...
      typedef std::deque<token_t>::iterator token_list_itr_t;
       ...
       generator(syntax_descriptor &desc)
      :
         base_itr_(0),
        s_itr_(0),
        s_end_(0),
   ...
   private:
      ...
      inline void scan_token()
      {
         ...
         else if(details::is_comment(syntax, s_itr_))
         {
             scan_comment(syntax);
         }
         else if(details::is_macro(syntax, s_itr_))
         {
             scan_macro(syntax);
         }
      ...
      inline void scan_comment(syntax_descriptor &desc)
      {
          token_t t;
          //The following comment styles are supported:
          // 1. // .... \n
          ...
          struct test
          {
             static inline bool comment_start(const char* str_itr_, comment_style &comments, int& mode, int& incr)
             {
                mode = 0;
                if( std::string_view(str_itr_,  comments.single_line_begin.size())
                        == comments.single_line_begin)
                    mode = 1;
                else if( std::string_view(str_itr_, comments.multi_line_begin.size())
                         == comments.multi_line_begin)
                    mode = 2;
                return (mode != 0);
             }

             static inline bool comment_end(const char *str_itr_, comment_style &comments,  int &mode)
             {
                 if(mode == 1) {
                     return ((*"\n") == *(str_itr_ + 1) );
                 }
                 ...
                 {
                     std::string_view str(str_itr_, comments.multi_line_end.size());
                     mode = comments.multi_line_end.size();
                     return (str == comments.multi_line_end);
                 }
                 return false;
          ...
          if (is_end(s_itr_) || is_end((s_itr_ + 1)))
             return;
          else if (!test::comment_start(s_itr_, desc.comments ,mode,increment))
             return;

          //s_itr_ += increment;

          while (!is_end(s_itr_) && !test::comment_end(s_itr_, desc.comments ,mode))
          {
             ++s_itr_;
          }
          if (!is_end(s_itr_))
      ...
      inline void scan_macro(syntax_descriptor &desc)
      {
          token_t t;
          const char *begin = s_itr_;
          ...
          while(!is_end(s_itr_) && (std::string_view(s_itr_, desc.macro.macro_end.size()) != desc.macro.macro_end))
          {
              if(lexertk::details::is_whitespace(*s_itr_)) {
                  t.set_macro(begin, s_itr_, base_itr_);
                  token_list_.push_back(t);
                  scan_whitespace();
                  begin = s_itr_;
                  continue;
              }
              if(lexertk::details::is_comment(desc, s_itr_)) break;
              ++s_itr_;
          }
          if(begin == s_itr_) return;
          t.set_macro(begin, s_itr_, base_itr_);
          token_list_.push_back(t);
      }
      ...
      inline void scan_operator()
      {
         ...
         if ('<' == *s_itr_)
            ...
            t.set_symbol( syntax, s_itr_,s_itr_ + 1,base_itr_);
         else if ('|' == *s_itr_)
            t.set_symbol( syntax, s_itr_,s_itr_ + 1,base_itr_);
         else
      ...
      inline void scan_symbol()
      {
         ...
         token_t t;
         t.set_symbol(syntax, begin,s_itr_,base_itr_, true);
         token_list_.push_back(t);
      }
