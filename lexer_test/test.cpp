...
namespace lexertk
{
   ...
   struct comment_style
   {
      ...
      inline bool is_keyword(std::string &str, const char **keywords)
      {
         if(keywords == nullptr) return false;
         size_t size = sizeof(keywords) / sizeof(const char *);
   ...
   namespace details
   {
      ...
      /*
      inline bool is_comment(const char c1, const char c2)
      {
          return ( (c1 == '#') || ( (c1 == '/') && ( (c2 == '/') || (c2 == '*') ) )  );
      }
      */

      inline bool is_comment(comment_style &com_style, const char *s_itr)
      {
          std::string single_str(s_itr, s_itr + com_style.single_line_begin.size());
          std::string multi_str(s_itr, s_itr + com_style.multi_line_begin.size());
          return( (single_str == com_style.single_line_begin) || (multi_str == com_style.multi_line_begin) );
      }
   ...
   struct token
   {
      ...
      template <typename Iterator>
      inline token& set_symbol(const char **keywords, const Iterator begin, const Iterator end, const Iterator base_begin = Iterator(0), bool can_be_keyword = false)
      {
         type = e_symbol;
         value.assign(begin,end);
         if(can_be_keyword && is_keyword(value, keywords) ) type = e_keyword;
         if (base_begin)
            position = std::distance(base_begin,begin);
         return *this;
      }
   ...
   public:
      ...
      typedef std::deque<token_t>::iterator token_list_itr_t;
       ...
       generator( comment_style comment_style_, const char **keywords_)
      :
         base_itr_(0),
        s_itr_(0),
        s_end_(0),
        comments(comment_style_),
        keywords(keywords_)
      {
         clear();
      }
   ...
   private:
      ...
      inline void scan_token()
      {
         ...
         //else if(details::is_comment(*s_itr_, *(s_itr_ + 1)))
         else if(details::is_comment(comments, s_itr_))
         {
             scan_comment(comments);
         }
      ...
      */
      ...
      inline void scan_comment(comment_style &comments)
      {
          token_t t;
          //The following comment styles are supported:
          // 1. // .... \n
          ...
          struct test
          {
             ...
             {
                mode = 0;
                if( std::string(str_itr_, str_itr_ + comments.single_line_begin.size())
                        == comments.single_line_begin)
                    mode = 1;
                else if( std::string(str_itr_, str_itr_ + comments.multi_line_begin.size())
             ...
             static inline bool comment_end(const char *str_itr_, comment_style &comments,  int &mode)
             {
                 ...
                 {
                     std::string str(str_itr_, str_itr_ + comments.multi_line_end.size());
          ...
          if (is_end(s_itr_) || is_end((s_itr_ + 1)))
             return;
          else if (!test::comment_start(s_itr_, comments ,mode,increment))
             return;

          //s_itr_ += increment;

          while (!is_end(s_itr_) && !test::comment_end(s_itr_, comments ,mode))
          {
             ++s_itr_;
          }
          if (!is_end(s_itr_))
      ...
      inline void scan_operator()
      {
         ...
         if ('<' == *s_itr_)
            ...
            t.set_symbol(keywords, s_itr_,s_itr_ + 1,base_itr_);
         else if ('|' == *s_itr_)
            t.set_symbol(keywords, s_itr_,s_itr_ + 1,base_itr_);
         else
      ...
      inline void scan_symbol()
      {
         ...
         token_t t;
         t.set_symbol(keywords, begin,s_itr_,base_itr_, true);
         token_list_.push_back(t);
      }
